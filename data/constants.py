#definindo a altura e largura da tela
TELA_ALTURA= 400
TELA_LARGURA = 700

TELA_TAMANHO = (TELA_LARGURA, TELA_ALTURA)

#definindo titulo
TITULO = "The Prince Dragon"

#definir as cores em RGB 
WHITE = (255, 255, 255)
RED = (255, 0, 0)
GRAY = (100, 100, 100)
NAVYBLUE     = ( 60,  60, 100)
GREEN        = (  0, 255,   0)
FOREST_GREEN = ( 31, 162,  35)
BLUE         = (  0,   0, 255)
SKY_BLUE     = ( 39, 145, 251)
YELLOW       = (255, 255,   0)
ORANGE       = (255, 128,   0)
PURPLE       = (255,   0, 255)
CYAN         = (  0, 255, 255)
BLACK        = (  0,   0,   0)
NEAR_BLACK    = ( 19,  15,  48)
COMBLUE      = (233, 232, 255)
GOLD         = (255, 215,   0)


#Fisica

#####
#MENU
#posição das imagens e textos no Menu
POS_TITULO= [50,0]
POS_BOTAO_PERM_TELA1 = [260,160]
POS_BOTAO_JOGAR= [355, 170]
POS_BOTAO_AJUDA= [215, 230]
POS_BOTAO_CREDITO= [355,230]
#POS_BOTAO3= [560, 350] #Creditos
POS_BOTAO_VOLTAR = [10, 350]
POS_BOTAO_PERM = [50, 280]
POS_BACKGROUND= [0,0]
POS_TEXT_JOGAR= [392, 175]
POS_TEXT_AJUDA= [246, 235]
POS_TEXT_CREDITO= [379, 235]
POS_TEXT_VOLTAR= [42, 355]
POS_TEXT_TIT_TELA2= [270,0]
POS_TEXT_TIT_TELA3= [240,0]
POS_TEXT_BOTAO_PERM1 = [42, 305]
POS_TEXT_BOTAO_PERM2 = [42, 318]
POS_TEXT_BOTAO_PERM1_TELA1 = [247, 175]
POS_TEXT_BOTAO_PERM2_TELA1 = [247, 188]


#tamanho dos Botoes
BOTOES_TAMANHO = (130, 40)
BOTAO_PERM_TAMANHO = (60, 60)

#textos Menu
TEXT_JOGAR = 'Jogar'
TEXT_AJUDA = 'Ajuda'
TEXT_TELA2 = 'Ajuda'
TEXT_TELA3 = 'Créditos'
TEXT_CREDITO = 'Créditos'
TEXT_BOTAO_VOLTAR = 'Voltar'
TEXT_BOTAO_PERM1 = 'Permanecer'
TEXT_BOTAO_PERM2 = 'nesta tela'

#fonts Menu
FONT_TITULO='DragonForcE'
FONT_BOTOES= 'JMH_Typewriter'

#tamanho da Fonte
FONT_TITULO_TAMANHO = 100
FONT_BOTOES_TAMANHO = 20
FONT_BOTAO_PERM = 12

#estados do cursor
BOTAO0= 'bot 0'
BOTAO_JOGAR= 'bot 1'
BOTAO_AJUDA= 'bot 2'
BOTAO_CREDITOS= 'bot 3'
BOTAO_VOLTA= 'bot 4'


#Estados do jogo (state_dict)
MAIN_MENU = 'main menu'
LOAD_SCREEN = 'load screen'
TIME_OUT = 'time out'
GAME_OVER = 'game over'
LEVEL1 = 'level1'

#telas do Menu

TELA1 = 'Menu'
TELA2 = 'Ajuda'
TELA3 = 'Creditos'
